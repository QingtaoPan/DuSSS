...
from .novel import objectives_usm_delloss
...
class my_novel(nn.Module):
    def __init__(self, batch_size, embed_dim=512):
        ...
        self.con_img_gau_encoder.apply(objectives_usm_delloss.init_weights)
        self.con_txt_gau_encoder.apply(objectives_usm_delloss.init_weights)
    ...
    def forward(self, image, image_aug, texts):
        ...
        loss_i2t = objectives_usm_delloss.compute_contrast_i2t(certain_value=certain_it, ret=ret, temp=self.temp)
        loss_t2i = objectives_usm_delloss.compute_contrast_t2i(certain_value=certain_it, ret=ret, temp=self.temp)
        #-----------------------------image-text-loss---------------------------------#

        #--------------------------------image-image-loss-----------------------------#
        certain_ii = torch.norm(image_feat+image_feat_m, p=2)
        certain_tt = torch.norm(text_feat+text_feat_m, p=2)
        loss_i2i = objectives_usm_delloss.compute_contrast_i2i(certain_value=certain_ii, ret=ret, temp=self.temp)
        loss_t2t = objectives_usm_delloss.compute_contrast_t2t(certain_value=certain_tt, ret=ret, temp=self.temp)
        #---------------------------------image-image-loss----------------------------#
