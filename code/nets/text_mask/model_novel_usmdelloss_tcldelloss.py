...
from .novel import objectives_usmdelloss_tcldelloss
...
def get_loss(hand_mask, soft_mask, image_feat, text_feat, image, mask_encoder, vision_proj):  # hard_mask[b, b, 56, 56]-0/1, soft_mask[b, b, 56, 56]-float, image_feat[b, 512, 56, 56], text_feat[b, 512], image[b, 3, 224, 224]
    ...
    # 图像级别loss
    tcli_loss = tcli_loss_fun(masks["pos"], image, text_feat, mask_encoder, vision_proj)  # [b, 1, 56, 56]-hard_mask, [b, 3, 224, 224]-image, [b, 512]-text_feat
...
class my_novel(nn.Module):
    def __init__(self, batch_size, embed_dim=512):
        ...
        self.con_img_gau_encoder.apply(objectives_usmdelloss_tcldelloss.init_weights)
        self.con_txt_gau_encoder.apply(objectives_usmdelloss_tcldelloss.init_weights)
    ...
    def forward(self, image, image_aug, texts):
        ...
        for text in texts:
            ...
            if text_embeds.shape[1] > 10:
                text_embeds = text_embeds[:, :10, :]  # [b, 10, 768]
        ...
        ################----------------------计算text_mask------------------------######################################################################
        image_embeds_b_512_14_14 = rearrange(image_embeds_b_197_512[:, 1:, :], "B (H W) C -> B C H W", H=14, W=14)  # [b, 512, 14, 14]
        image_embeds_b_512_56_56 = self.GD_Dec(image_embeds_b_512_14_14)  # [b, 512, 56, 56]
        image_feat_norm = F.normalize(image_embeds_b_512_56_56, dim=1)  # [b, 512, 56, 56]
        text_feat_norm = F.normalize(text_feat, dim=1)  # [b, 512]
        simmap = torch.einsum("bchw,nc->bnhw", image_feat_norm, text_feat_norm)  # [b, b, 56, 56]
        hard_mask, soft_mask = self.masker(simmap, deterministic=False)
        loss_mask = get_loss(hard_mask, soft_mask, image_feat_norm, text_feat_norm, image, self.visual_encoder_m, self.vision_proj_m)

        mask_final = self.apply_pamr(image, soft_mask)
        mask_final = self.kp_branch(image_feat_norm, text_feat_norm, mask_final)
        mask_final = F.interpolate(mask_final, (224, 224), mode='bilinear')  # [B, N, H, W]
        ...
        #-----------------------------image-text-loss---------------------------------#
        certain_it = torch.norm(image_feat+text_feat_m, p=2)
        loss_i2t = objectives_usmdelloss_tcldelloss.compute_contrast_i2t(certain_value=certain_it, ret=ret, temp=self.temp)
        loss_t2i = objectives_usmdelloss_tcldelloss.compute_contrast_t2i(certain_value=certain_it, ret=ret, temp=self.temp)
        #-----------------------------image-text-loss---------------------------------#

        #--------------------------------image-image-loss-----------------------------#
        certain_ii = torch.norm(image_feat+image_feat_m, p=2)
        certain_tt = torch.norm(text_feat+text_feat_m, p=2)
        loss_i2i = objectives_usmdelloss_tcldelloss.compute_contrast_i2i(certain_value=certain_ii, ret=ret, temp=self.temp)
        loss_t2t = objectives_usmdelloss_tcldelloss.compute_contrast_t2t(certain_value=certain_tt, ret=ret, temp=self.temp)
        #---------------------------------image-image-loss----------------------------#
