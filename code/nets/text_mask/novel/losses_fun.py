...
# 特征级损失
def tclf_loss_fun(s1_image_emb, mask, s1_text_emb):  # s1_image_emb[b, 512 56, 56], mask:[b, b, 56, 56]--float, s1_text_emb[b, 512]
...
# 图像级别损失-输入数据类型为half()
def tcli_loss_fun(mask_pos, image, text_emb, mask_encoder, vision_proj):  # [b, 1, 56, 56]-hard_mask, [b, 3, 224, 224]-image, [b, 512]-text_feat
    pos_mask = F.interpolate(mask_pos, size=image.shape[2:])  # 输入-->mask["pos"]:硬标签[b, 1, 56, 56],  输出-->[b, 1, 224, 224]
    mask_image = pos_mask*image
    mask_image = mask_image.to(image.device)
    masked_img_emb = mask_encoder(mask_image)  # 输入:pos_mask*image[b, 3, 224, 224],  输出:[b, 197, 768]
    masked_img_emb = F.normalize(vision_proj(masked_img_emb[:,0,:]),dim=-1)  # [b, 512]
