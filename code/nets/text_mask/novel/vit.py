...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, register_blk=-1):
        ...
        cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks  [b, 1, 768]
        x = torch.cat((cls_tokens, x), dim=1)  # [b, 257, 768]
  
        x = x + self.pos_embed[:,:x.size(1),:]  # [b, 257, 768]
        x = self.pos_drop(x)  # [b, 257, 768]
