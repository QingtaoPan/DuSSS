...
class DisAttention(nn.Module):
    ...
    def forward(self, x, mask=None): # x[b, 197, 768]
        B, N, C = x.shape  # b, 197, 768
        qkv = (
            self.qkv(x)
            .reshape(B, N, 3, self.num_heads, C // self.num_heads)  # [b, 197, 3, 12, 768/12]
            .permute(2, 0, 3, 1, 4)  # [3, b, 12, 197, 768/12]
        ) # (3, B, mu_heads_num+logsig_heads_num, n, dim_heads)
        q, k, v = (
            qkv[0],  # [b, 12, 197, 768/12]
            qkv[1],  # [b, 12, 197, 768/12]
            qkv[2],  # [b, 12, 197, 768/12]
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, C).reshape(B, N, 2, int(C/2))  # [b, 12, 197, 768/12]-->[b, 197, 12, 768/12]-->[b, 197, 768]-->[b, 197, 2, 768/2]

        mu = x[:,:,0,:]  # [b, 197, 768/2]
        logsigma = x[:,:,1,:]  # [b, 197, 768/2]
...
class DisTrans(nn.Module):
    ...
    def forward(self, x, mask=None):  # x[b, 197, 768]
        x_ = self.norm1(self.act(self.fc(x)))  # x[b, 197, 768]
        mu, logsigma, attn = self.attn(x_, mask=mask)  # x[b, 197, 768]-->
